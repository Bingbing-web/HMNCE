...
def load_data(dataset, bfs_level=3, relabel=False, data_root=None):
    """Load a link-prediction dataset from an explicit dataset root.

    A valid dataset directory contains ``entities.dict``, ``relations.dict``,
    ``train.txt``, ``valid.txt``, ``test.txt`` and ``neg.txt``.  This replaces
    the historical fixed list of dataset names and the ``../datasets`` path.
    """
    root = Path(data_root or os.environ.get("HMNCE_DATA_ROOT", Path(__file__).resolve().parents[1] / "datasets"))
    directory = root / dataset
    if directory.is_dir():
        return load_link(dataset, root)
...
class RGCNLinkDataset(object):
    ...
    def __init__(self, name, data_root=None):
        self.name = name
        self.root = Path(data_root or os.environ.get("HMNCE_DATA_ROOT", Path(__file__).resolve().parents[1] / "datasets"))
        self.dir = str(self.root)

        zip_path = os.path.join(self.dir, '{}.zip'.format(self.name))
        self.dir = os.path.join(self.dir, self.name)
        if os.path.isdir(self.dir):
            print('Using existing dataset folder {}'.format(self.dir))
        else:
...
def load_link(dataset, data_root=None):
    data = RGCNLinkDataset(dataset, data_root=data_root)
    data.load()
    return data
