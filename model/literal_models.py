...
class TransELiteral_gate(nn.Module):
    def __init__(self, num_ents, num_rels, numerical_literals, missing, params=None):
        ...
        self.numerical_literals = torch.from_numpy(numerical_literals).cuda().float()
        ...
        self.missing = torch.from_numpy(missing).cuda().float()
        self.numerical_literals1 = torch.from_numpy(numerical_literals).cuda().float()
