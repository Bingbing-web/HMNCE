...
class RAKGE(nn.Module):
    def __init__(self, num_ents, num_rels, numerical_literals, params=None):
        ...
        self.device = getattr(self.p, "device", torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        ...
        self.numerical_literals = torch.from_numpy(numerical_literals).to(self.device)
    ...
    def forward(self, g, e1, rel, e2_multi, neg, n_label):
        ...
        if self.training:
            ...
            # Generator
            rand = torch.rand(e2_multi.shape[0], e2_multi.shape[1], device=e2_multi.device)
            ...
            alpha = torch.rand(e2_multi.shape[0], 1, device=e2_multi.device)
            beta = torch.rand(e2_multi.shape[0], 1, device=e2_multi.device)
